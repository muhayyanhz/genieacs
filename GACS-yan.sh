...
run_command "apt install -y curl gnupg build-essential logrotate" "Installing base packages ($(( ++current_step ))/$total_steps)"
...
run_command "echo 'deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb.gpg ] https://repo.mongodb.org/apt/debian bullseye/mongodb-org/4.4 main' > /etc/apt/sources.list.d/mongodb-org.list" "Adding MongoDB repo ($(( ++current_step ))/$total_steps)"

run_command "apt update -y" "Updating repo ($(( ++current_step ))/$total_steps)"

run_command "apt install -y mongodb-org" "Installing MongoDB ($(( ++current_step ))/$total_steps)"

run_command "systemctl enable mongod" "Enabling MongoDB ($(( ++current_step ))/$total_steps)"

run_command "systemctl start mongod" "Starting MongoDB ($(( ++current_step ))/$total_steps)"
...
for service in cwmp nbi fs ui
do
run_command "systemctl enable genieacs-$service && systemctl start genieacs-$service" "Starting genieacs-$service ($(( ++current_step ))/$total_steps)"
done
...
for service in mongod genieacs-cwmp genieacs-nbi genieacs-fs genieacs-ui
do
